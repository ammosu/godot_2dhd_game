...
Runtime: `scripts/gameplay/resident_art.gd` selects the view relative to camera orientation and world heading. Indoor residents stay upright and face the player during dialogue, then resume their idle heading. Patrols use the same art, advance their own walking clock, yield to the player, and keep the existing routes. The fixed street roster is Flo (east plaza), Mira (west market), and Owen (north street), configured in HouseCatalog.STREET_PATROLS. Each has a named interaction prompt and distinct repeatable outdoor dialogue, sharing the existing dialogue UI and input lock. Dialogue pauses all patrols and faces the player; patrols resume afterward. Identity/lines remain stable across map reloads without adding save fields or rewards.
