...
`godot --headless --path . --fixed-fps 60 --script tests/enemy_awareness_test.gd` 驗證視野、背後接近、牆壁遮蔽、支援、不連鎖引怪、最後已知位置、追擊記憶、脫戰與受擊警報；成功標記 `ENEMY_AWARENESS_TEST_PASS`。不寫正式存檔。

## 野外怪物強度

野外採固定強度，不隨玩家等級自動膨脹：蝙蝠 42 HP／攻擊 11，鬥士狼 72 HP／攻擊 13，術士 54 HP／攻擊 18；舊道坡道的精英狼 100 HP／攻擊 14。初始戰士攻擊 18，單靠普攻分別需 3／4／3／6 下。術士移動較慢；蝙蝠／一般狼／精英狼／術士的蓄力分別為 0.45／0.55／0.60／0.65 秒，攻擊間隔為 1.10／1.35／1.25／1.70 秒（從開始蓄力計時）。精英狼蓄力稍長，但兩次出招之間的空檔較短。升級與裝備仍可縮短戰鬥。

野外怪物採不同循環：蝙蝠「三次普攻 → 撲擊」、一般狼「兩次普攻 → 重擊」、精英狼「一次普攻 → 重擊」、術士「兩發魔彈 → 蓄力法術」：近戰普攻前搖 0.16 秒、術士魔彈 0.22 秒，普攻使用 75% 攻擊力（四捨五入後扣防禦），不顯示技能紅圈；循環末擊才使用上列蓄力時間、完整攻擊力與紅圈。普攻的出招間隔為上列基準的 85%，技能為 120%，形成連攻與重招後的反擊空檔。成功出手即推進循環，閃避不影響計數；技能被打斷後回到普攻，脫戰回家後重置。首領維持既有專屬技能循環。

循環回歸：`godot --headless --path . --fixed-fps 60 --script tests/field_attack_cycle_test.gd`，檢查四種敵人的兩輪循環、傷害、預警、打斷與脫戰重置。

普攻只可打斷輕型蝙蝠；技能可打斷一般狼與術士。成功打斷後有 1.5 秒打斷恢復，避免連打使敵人永遠無法出手；首領維持既有不可打斷規則。閃避與離開紅圈仍能避開攻擊。

回歸：`godot --headless --path . --fixed-fps 60 --script tests/field_combat_test.gd`，驗證強弱差異、普攻存活、技能打斷、打斷恢復、閃避、掉落與存讀檔。另跑下方自動戰鬥與 `tests/class_field_test.gd`、`tests/ashen_crypt_test.gd`。
