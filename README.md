...
本版諾亞與長老是試煉援軍，每場恢復初始 HP／MP，尚無永久隊伍編成；旅人 HP／MP 與藥水仍由 GameState 保存。旅人倒下但隊伍獲勝時以 1 HP 起身。戰鬥中途不存檔，因此無新增存檔欄位，version 1 存檔仍相容。諾亞與長老已各有待機、攻擊／施法、受擊、防禦四個原創關鍵姿勢，並非完整逐格動畫；來源與提示詞見 `assets/generated/ALLY_COMBAT.md`。
...
室內沿用既有存檔欄位 `current_map`／`saved_position`，以 `house_01`～`house_08` 識別住宅；沒有新增序列化欄位或改變資料格式，原 version 1 村莊／遺跡存檔仍相容。
