...
func _actor(label: String, team: int, hp: int, mp: int, attack: int, defense: int, point: Vector2, art: String) -> Dictionary:
	return {"name": label, "team": team, "hp": hp, "max_hp": hp, "mp": mp, "max_mp": mp, "attack": attack, "defense": defense, "position": point, "art": art, "guard": false}
...
func preview(action: String, target: int) -> Array[int]:
	...
	if action == "guard" or action == "potion":
		return [current]
	if target < 0 or target >= actors.size() or int(actors[target].hp) <= 0 or actors[target].team == actors[current].team:
		return empty
	if action != "magic":
		return [target]
...
func resolve(action: String, target: int) -> Dictionary:
	...
	if living(1).is_empty():
		...
		winner = 1
	return {"targets": affected, "damage": damage, "action": action, "caster": current}
